...
function TiltCard({ children, className }) {
  ...
  const handleMouseMove = (e) => {
    ...
    if (glowRef.current) {
      ...
      glowRef.current.style.background = `radial-gradient(300px circle at ${x}px ${y}px, rgba(0,255,136,0.08), transparent 60%)`
    }
  }
  ...
  return (
    ...
    >
      <div
        ref={glowRef}
        className="absolute inset-0 rounded-xl pointer-events-none transition-opacity duration-500 z-0"
        style={{ opacity: 0 }}
      />
      {children}
    </div>
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-5">
          {projects.map((project, i) => (
            ...
            >
              <TiltCard className="group relative rounded-xl p-6 border border-white/[0.06] bg-white/[0.02] backdrop-blur-md flex flex-col h-full hover:border-accent/20 transition-all duration-500">
                ...
                {/* Top accent line */}
                <motion.div
                  className="absolute top-0 left-6 right-6 h-px bg-gradient-to-r from-transparent via-accent/40 to-transparent"
                ...
                <div className="relative z-10 flex flex-col flex-grow">
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="font-heading text-text-primary font-semibold text-lg group-hover:text-accent transition-colors duration-300">{project.title}</h3>
                    <div className="flex gap-3">
                      {project.github && (
                        <a href={project.github} target="_blank" rel="noreferrer" className="text-text-secondary hover:text-accent transition-colors duration-300 hover:scale-110 transform" aria-label="GitHub">
                      ...
                      {project.live && (
                        <a href={project.live} target="_blank" rel="noreferrer" className="text-text-secondary hover:text-accent-purple transition-colors duration-300 hover:scale-110 transform" aria-label="Live demo">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {project.tech.map((t) => (
                      <span key={t} className="font-mono text-accent/70 text-[11px] tracking-wide hover:text-accent transition-colors duration-200">{t}</span>
                    ))}
                  </div>
                </div>
              </TiltCard>
